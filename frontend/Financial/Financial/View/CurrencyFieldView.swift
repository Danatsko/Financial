...
public struct CurrencyFieldView: View {
...
struct CurrencyInputField: UIViewRepresentable {
    ...
    class Coordinator: NSObject, UITextFieldDelegate {
        ...
        func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
            // Поточний текст у полі
            let currentText = textField.text ?? ""
            // Нова довжина тексту після змін
            let newLength = currentText.count + string.count - range.length

            // Перевірка: чи не перевищує нова довжина максимальний ліміт
            if newLength > input.maxLength {
                return false // Заборонити зміну тексту
            }

            // Якщо replacement string порожній (натиснуто backspace)
            if string.isEmpty {
                if input.value == 0 {
